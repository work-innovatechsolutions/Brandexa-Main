...
class Awaiken_ElementsKit_Lite {
	...
	public function onboarded_status() {
		...
		if ( get_option('awaiken_default_ekit_settings') == 1 ) {
			...
			$elemkit_options[ 'widget_list' ][ 'advanced-toggle' ][ 'status' ] = 'active';
			...
			$elemkit_options[ 'widget_list' ][ 'fancy-animated-text' ][ 'status' ] = 'active';
	...
	public function admin_head_script() { 
		...
		$license_menu 	= 	apply_filters( 'artistic_theme_enable_elementskit_license_menu', false );
		$styles			=	array();
		...
		if($license_menu === false) {
			...
			$styles[] = '#adminmenu a[href^="admin.php?page=elementskit-lite_wpmet_plugins"] { display:none; }';
			$styles[] = '#adminmenu a[href^="admin.php?page=forms"] { display:none; }';
		}
